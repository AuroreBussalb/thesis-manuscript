...
\subsection{Calcul de la significativité de la taille d'effet totale}
...
D'autres valeurs peuvent ensuite être calculées pour compléter l'analyse, comme par $I^2$ qui estime l'hétérogénéité des \gls{es}-inter-groupes. 

\subsection{Analyse des biais}

Lors de la sélection des études à intégrer dans l'analyse, un biais de publication peut s'immiscer : les auteurs d'études scientifiques ont tendance à 
plutôt publier des résultats statistiquement significatifs et en faveur de ce qu'ils veulent démontrer. L'analyse visuelle d'un \textit{funnel plot} 
est une méthode couramment utilisée pour détecter un éventuel biais de publication lors de la sélection des études et une hétérogénéité parmi elles 
\citep{Sterne2011}. 

Il s'agit d'un nuage de points de la précision de chaque \gls{es}-inter-groupes en fonction des \gls{es}-inter-groupes. L'inverse de l'\gls{et} est couramment 
...
zéro (seuil de signifiance à 5\%), alors le \textit{funnel plot} est asymétrique.
...
Finalement, 3 études ont été identifiées lors de la recherche menée le 12 février 2018 dont les résultats sont présentés dans \citet{Bussalb2019clinical}  : 
\begin{itemize}
...
Une mise à jour de ce travail a été menée et est décrite en \ref{need_to_update_meta_analysis}.
...
\begin{figure}[h!]
  ...
  \caption{Exemples d'évolution de la p-value des \gls{est} au fur et à mesure que la taille de l'échantillon augmente. 
	En \textbf{A}, la p-value se stabilise sous le seuil de significativité suite à l'ajout de nouveaux sujets ; en \textbf{B}, la p-value 
