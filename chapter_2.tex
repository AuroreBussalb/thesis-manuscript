...
par l'erreur type de la vraie population qui est supposée être identique pour les deux groupes $\delta$ \citep[Chapitre~4]{Borenstein2009}. Ce biais peut être supprimé 
...
\subsection{Calcul de la signifiance statistique de la taille d'effet totale}
...
p = 2(1 - \Phi(\abs{Z})),
\end{equation} 
où $\Phi$ est la fonction de distribution cumulative de la loi normale standardisée (\textit{ie.} centrée et normée).
...
\subsubsection{Mettre en évidence l'importance des mises à jour} \label{methods_importance_to_update_meta_analysis}
...
La $p$-value est très largement utilisée mais est également de plus en plus contestée du fait de sa propension à être mal interprée \citep{Halsey2019}. 
Ainsi, certains chercheurs se tournent vers des substituts tels que le facteur de Bayes \citep{Morey2016} ou l'\gls{aic} \citep{Mcelreath2018}. L'évolution de ces valeurs avec l'inclusion de 
nouvelles études serait intéressante à étudier. 

Pour conclure, la mise à jour des méta-analyses peut avoir une incidence sur la signifiance statistique de la $p$-value, notamment dans le cas où peu d'études et donc de sujets
...
% trouver de la biblio à ajouter
