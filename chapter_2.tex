...
\begin{itemize}
    ...
    \item la différence moyenne standardisée (\textit{standardized mean difference} en anglais), d :
        ...
        \text{d} = \frac{\bar{X}_{1} - \bar{X}_{1}}{\sqrt{\frac{(n_1 - 1)S_1^2 + (n_2 - 1)S_2^2} {n_1 + n_2 - 2}}},
        \end{equation} 
...
que la p-value du \gls{est} passe sous le seuil de signicativité de 0.05 \citep{Head2015, Coffman2015}. En effet, l'arrêt prématuré des résultats peut
...
davantage fiables une fois que la p-value converge. Ces deux configurations sont représentées à la Figure~\ref{Figure:meta_review_pvalue_example}
...
incluses au fur et à mesure selon leur année de publication est tracée. 
...
A première vue, la plus forte fluctuation correspond à la p-value des \gls{est} obtenues pour les évaluations des enseignants (en bleu sur le graphique \textbf{B} de 
la Figure~\ref{Figure:meta_analysis_evolution_pvalue}). En effet, en 2013 la p-value atteint un pic à 0.6, cependant depuis 2014 elle diminue et tend de plus 
en plus vers la significativité, sans converger pour le moment, ce qui laisse penser que plus d'études sont nécessaires afin de vraiment pouvoir conclure quant 
à l'efficacité du \gls{nfb} selon les enseignants.

En ce qui concerne les p-values des \gls{est} obtenus grâce aux évaluations des parents, elles varient mais ne dépassent pas le seuil de significativité. L'évolution
des p-values pour le groupe du protocole \gls{nfb} standard (en orange sur le graphique \textbf{A} de la Figure~\ref{Figure:meta_analysis_evolution_pvalue}) 


% Biais généré par l'arrêt des résultats : arrêter la collecte de données dès que p-value passe sous le seuil désiré (5%) et trouver des refs qui dénonce ce genre
% p-hacking
\citep{Head2015} % and stopping data exploration if an analysis yields a significant p-value

% images pour illuster : p-value qui diverge au cours du temps et une p-value qui converge
% c'est pourquoi on s'intéresse à l'evolution de la p-value au cours du temps, car une méta-aalyse n'est qu'une photo à l'instant t, on ne peut en tirer une
% conclusion que quand la valeur s'est stabilisée
% plot evolution des p-values (mettre aussi les mises à jour (faire des points diff)
% p-value fluctue à chaque mise à jour, pas de conclusion valide
% on illustre ce fait avec la meta-analyse de frontiers car la recherche date de 1 an et demi
% Nouveaux résultats
% et en effet il y a des changements ce qui valide ce qui a été dit précédement

% dire que pour les parents c'est significatif mais ça fluctue
