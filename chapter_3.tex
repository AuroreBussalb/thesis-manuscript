...
\item \emph{l'implémentation du \gls{nfb} :} 
    \begin{itemize}
    \item le protocole utilisé (\gls{scp}, \gls{smr}, l'augmentation du rythme thêta, l'augmentation du rythme bêta dans les aires centrale ou frontale 
    et la diminution du rythme thêta), 
    ...
    \item le type de seuillage pour les récompenses discrètes (le seuil qui permet l'attribution des récompenses peut avoir une valeur fixe tout au long du traitement ou bien variable
		...
		\item l'individualisation des bandes de fréquences basée sur l'\gls{iapf} (l'\gls{iapf} est la valeur du pic dans la bande alpha qui diffère selon les individus), 
...
\item \emph{la qualité de l'acquisition :} 
    ...
    \item la présence de plus d'une électrode d'enregistrement de l'\gls{eeg} (c'est à dire les électrodes qui enregistrent l'\gls{eeg} et d'où 
		est extrait le neuromarqueur qui mènera au \textit{feedback}),
		\item la qualité de l'\gls{eeg}. Cette dernière est représentée par un indicateur allant de 1 à 3, calculé sur les critères suivants : 
        \begin{description} 
...
appliquée aux enfants \gls{tdah} \citep{Arns2009, Maurizio2014, Strehl2017}. Dans le cas de l'analyse réalisée ici, lorsque l'\gls{es}-intra-groupe est négatif et que sa valeur 
absolue est élevée, le traitement étudié est efficace. 
...
\subsection{Analyse systématique des biais} \label{saob_method}
...
\end{equation}
avec $E$ l'espérance statistique, $Y$ la variable aléatoire, $\mu$ la moyenne de la population et $\sigma$ l'écart-type de la population.
...
La variance de chaque coefficient, nécessaire pour effectuer le t-test, est estimée à l'aide des éléments 
diagonaux de la matrice de covariance des coefficients (\textit{coefficient covariance matrix}) \citep{Seabold2010}.
Pour les coefficients significativement différents de 0, le facteur qui leur est associé est supposé avoir une influence sur l'efficacité du \gls{nfb}. Par ailleurs, 
...
Afin que les seuils calculés par le \gls{dt} aient un sens, les variables indépendantes n'ont ici pas été standardisées. Dans le cas de la \gls{saob}, 
...
\begin{table}[t]
  \centering
  \caption[Liste des études incluses dans la \glsfirst{saob}.]{Liste des études incluses dans la \gls{saob}: a) études incluses dans \citet{Cortese2016}
...
\begin{itemize}
	\item le protocole visant l'augmentation du rythme bêta dans les aires frontales,
...
\begin{table}[h!]
  \centering
  \caption[Résultats de la \gls{saob}.]{Résultats de la régression linéaire pondérée (\gls{wls}), de la régression linéaire régularisée (\gls{lasso}) et de l'arbre de décision (\gls{dt}). Pour la \gls{wls}, une $p$-value $<$ 0.05 
...
\begin{figure}[h!]
  ...
  \caption[Arbre de décision]{Arbre de décision obtenu : \gls{es} correspond à l'\gls{es}-intra-groupe et $n$ au nombre d'observations. L'importance des variables indépendantes
...
Quatre facteurs sont retournés seulement par une méthode : le nombre de sessions, l'augmentation du rythme bêta dans la zone centrale, l'enregistrement
...
médicaments pendant le traitement par \gls{nfb}, les protocoles \gls{smr}, diminution de thêta et \gls{scp}, et le rejet
...
Tout d'abord, le type de protocole \gls{nfb} n'a été identifié, étonnamment, par aucune méthode à l'exception de l'augmentation du rythme bêta en central qui l'a été par une méthode. 
...
n'utilisent pas de carte), il a été exclu de l'analyse. 

L'impact négatif de la phase de transfert va à l'encontre de la littérature \citep{Arns2014}, et peut être un effet défavorable ou un effet nocebo (effet 
clinique négatif sans cause thérapeutique spécifique). Ce facteur n'avait été identifié que par une seule méthode dans \citet{Bussalb2019clinical} 
comme souligné en \ref{comparison_with_bussalb_2019}, ainsi une mise à jour avec un plus grand nombre de données est nécessaire pour confirmer l'impact négatif de la 
phase de transfert, préalable indispensable pour pouvoir ensuite trancher entre les deux modes d'explication.
...
de fréquences varient d'un individu à l'autre et sont aussi dépendantes de l'âge. Par ailleurs, les résultats paraissent prometteurs selon 
...
Selon \citet{Arns2014}, le seuillage automatique est contraire aux principes de la théorie de l'apprentissage : ce type de seuillage ne récompense
...
\subsection{Comparaison avec les résultats de Bussalb et al., 2019b} \label{comparison_with_bussalb_2019}
...
démontrerait plutôt un effet défavorable ou nocebo de cette dernière. 
