...
La régression linéaire a pour but d'estimer les coefficients de régression qui lient les facteurs aux \gls{es}-intra-groupe \citep{Montgomery2012}. Ici, la régression est pondérée pour, d'une part, 
...
Les coefficients $\hat{\beta}_{j}$, $0 \leq j \leq p$ sont obtenus en minimisant la somme pondérée des carrés :
\begin{equation}
\label{eq:factors_lasso-minimization}
\hat{\beta} = \argmin_\beta \sum_{i=1}^{n} w_{i} \abs{ y_i - \sum_{j=0}^{p}\beta_{j}x_{ij} }^2,
\end{equation} 
avec $n$ le nombre d'observations et p le nombre de variables indépendantes. Le poids $w_{i}$ associé à chaque observation $i$ est défini comme suit : 
\begin{equation}
\label{eq:weight_WLS}
...
\begin{figure}[h!]
  ...
  \caption{Comparaison des régressions non pondérée (\gls{ols}) en \textbf{A} et pondérée (\gls{wls}) en \textbf{B} sur des données factices. La droite rouge correspond à la droite de régression
	estimée : en \textbf{A} elle a pour équation $y = 0.30x + 3.46$ et en \textbf{B} $y = 0.39x + 1.87$.}
...
la valeur de $\lambda$ dans le cas des régressions \gls{lasso} (en \textbf{A}) et \textit{Ridge} (en \textbf{B}) sur des données factices. Lorsque $\lambda = 0$, on se retrouve
dans le cas de l'\gls{ols} évoqué en \ref{wls_part}.
...
L'analyse présentée dans ce chapitre est la mise à jour des résultats présentés dans \citet{Bussalb2019clinical}. Etant donné qu'un nombre encore limité d'études
...
% Ajouter graphe du nombre de méthodes en fonction de l'année de publication, choisir le nombre d'étude de départ en fonction de si les hypothèses de
% travail sont satisfaites ou non (autre critère ?)
% étudier SMR, eeg quality2, nombre de session et durée du traitement
% car on veut 1 facteur par nombre de méthode (résultats de Bussalb2020), durée du traitement a aussi été identifiée par 3 méthodes dans Bussalb2019
% nombre de sessions très discuté dans la littérature
% EEG quality 2 car retourné par 3 méthodes dans Bussalb2019 et facteur intéressant
% SMR protocole couramment utilisé
