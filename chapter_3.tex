...
et le Dr. Louis Mayaud, ont permis de déterminer les facteurs dont il serait intéressant d'étudier l'influence \citep{Vernon2004, Arns2014, Cortese2016, Enriquez2017}. Ces facteurs sont spécifiques
à l'entrainement par \gls{nfb} et sont définis avec précision en \ref{principe_nfb}.
...
\item \emph{la population :} 
    ...
    \item la prise de psychostimulants durant le traitement par \gls{nfb}, 
		...
		\item le degré d'engagement dans l'entrainement par \gls{nfb} qui se traduit notamment par l'observance thérapeutique.
    \end{itemize}
\item \emph{l'implémentation du \gls{nfb} :} 
    ...
    et la diminution du rythme theta), 
		\item la présence d'une phase de transfert lors de l'entrainement par \gls{nfb}, 
		\item l'utilisation d'une méthodologie de transfert d'apprentissage (pour s'entrainer à la maison ou à l'école), 
    \item le type de seuillage pour les récompenses discrètes, 
		\item le nombre de sessions de \gls{nfb}, 
		\item la durée et la fréquence des sessions, 
		\item la durée du traitement, 
		\item l'individualisation des bandes de fréquence basée sur l'\gls{iapf}, 
		\item le couplage du \gls{nfb} avec l'\gls{emg}-Biofeedback.
		\end{itemize}
\item \emph{la qualité de l'acquisition :} 
    \begin{itemize}
    \item la présence de plus d'une électrode active et la qualité de l'\gls{eeg}. Cette dernière est représentée par un indicateur allant de 1 à 3, calculé sur les critères 
    suivants : 
        \begin{description} 
