...
un \gls{gmm}. Un \textit{Gaussian Mixture Model} est un modèle probabiliste qui suppose que toutes les observations proviennent d'un mélange 
...
créé pour modéliser les données. Les valeurs de \gls{srntbr} sont approximées par un \gls{bgmm} avec un nombre croissant de composantes. Il en va de même 
pour l'utilisation du \gls{gmm}.
...
modèle est testée grâce à un test $\chi^2$ sur la déviance \citep{James2013}. La déviance est une mesure de la qualité de l'ajustement (\textit{goodness-of-fit} en anglais) 
d'un modèle statistique et est définie comme suit :
\begin{equation}
...
horizontale est tracée de façon à couper cette ligne verticale. Le nombre de lignes verticales coupées par la ligne horizontale tout juste tracée correspond 
au nombre optimal de groupes.

\subsubsection{Partitionnement basé sur les densités : DBSCAN}
\gls{dbscan} est un algorithme de partitionnement de données qui s'appuie sur la densité des \textit{clusters} estimés \citep{Ester1996}. Deux paramètres sont entrés dans l'algorithme :
...
Une méthode pour déterminer la valeur $\epsilon$ optimale est de tracer le \textit{k-distance plot} obtenu par la méthode des k plus proches voisins 
(\textit{K-nearest neighbors} en anglais) en prenant $k = MinPts$ \citep{James2013, Goldberger2005}. Etant donné que les points bruités 
...
est utilisée pour en déduire la valeur du seuil conduisant à la meilleure discrimination entre les deux \textit{clusters}. De plus, une courbe \gls{roc} 
...
(\gls{tpr} en anglais) \citep{James2013} :
...
l'\gls{auc} : plus cette aire est grande, plus la courbe \gls{roc} s'éloigne du partionnement aléatoire.

Si le \gls{gmm}, la méthode de Ward et le \gls{dbscan} identifient également deux \textit{clusters}, le seuil de discrimination obtenu par ces approches
...
Les résultats des \gls{bgmm} avec un nombre croissant de composantes sont représentés à la Figure~\ref{Figure:tbr_bgmm}. 
...
\begin{figure}[h!]
  ...
  \caption{Tracés des distributions de \gls{srntbr} (\textit{rug plot}) avec \textbf{A)} 1-mode, \textbf{B)} 2-modes et \textbf{C)} 3-modes du \gls{bgmm} superposés. P-value (1-2) correspond
	...
	Significativité symbolisée par ** (seuil fixé à 0.01). Le seuil \gls{srntbr} est précisé pour le modèle .
...
Le \gls{gmm} arrive à la même conclusion : la distribution des \gls{srntbr} est mieux ajustées par deux modes (p-value pour la comparaison
...
% expliquer qu'on met le seuil au carré faire une ref aux figures où le seuil est écrit. 
