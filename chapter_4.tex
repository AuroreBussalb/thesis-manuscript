...
inférieures à $40\text{k}\Omega$ et le niveau de contamination électromagnétique devait rester inférieur à 1/3 de l'énergie totale du signal. Ces deux points étaient vérifiés 
grâce à des étapes de contrôle qui devaient être validées pour pouvoir passer à l'enregistrement de l'\gls{eeg}.
...
variance (autrement dit l'électrode doit être déconnectée et n'enregistre rien) : dans les deux cas il est interpolé par \textit{spherical spline interpolation} \citep{Freeden1984, Perrin1989}. 
Ces valeurs ont été choisies
empiriquement en se basant sur la distribution des variances mais n'ont pas été validées de manière plus précise.  

Les données sont re-référencées sur l'électrode de la mastoïde gauche (électrode 57 de l'\gls{egi}-128) pour correspondre au référencement de la base
...
Tout d'abord, les données artefactées sont exclues en utilisant une méthode basée sur la géométrie Riemannienne : le \gls{rpf} \citep{Barachant2013, Barthelemy2019}. 
...
Afin d'augmenter la sensibilité et la spécificité de cette méthode de rejet, plusieurs petites patates définies sur certains canaux peuvent travailler en parallèle \citep{Barthelemy2019}, 
...
plus grande qu'un seuil obtenu grâce à une référence d'\gls{eeg} propre. 

Ici, la calibration des patates n'a pas été réalisée à l'aide d'une référence d'\gls{eeg} propre, mais en utilisant les signaux \gls{eegi} desquels les artefacts doivent être rejetés : 
il s'agit d'un \textit{self-calibrated} \gls{rpf} \citep{Barthelemy2019}. Cette technique peut être appliquée si et seulement si les signaux \gls{eegi} sont de bonne qualité, ce qui est 
supposé être le cas ici étant donné que tous les enregistrements ont été effectués dans un environnement contrôlé et sous la supervision d'un spécialiste \citep{Bioulac2019, Langer2017, Alexander2017}. 
...
L'\gls{iapf} permet de définir des bandes de fréquences personnalisées pour chaque sujet qui ont été choisies 
...
Dans notre cas le \gls{tbr} est défini comme le ratio de la puissance dans la bande de fréquences thêta et de la bande de fréquences bêta obtenues 
...
de l'enregistrement, puis le maximum sur ces deux électrodes est retenu en accord avec les directives du comité scientifique de NEWROFEED.

Enfin, cette valeur de \gls{tbr} est normalisée pour les analyses de partitionnement qui vont suivre. En effet, deux des méthodes utilisées pour modéliser la distribution des \gls{tbr},
le \glsfirst{bgmm} et le \gls{gmm} détaillés par la suite, requièrent que les données soient distribuées selon un mélange de Gaussiennes \citep{Blei2006}, 
ainsi on normalise les données afin que chaque mode potentiel soit effectivement distribué normalement. 
...
\begin{figure}[h!]
  ...
  \caption[Comparaison de techniques de normalisation des valeurs de \gls{tbr}.]{Comparaison de techniques de normalisation des valeurs de \gls{tbr}. 
	...
	log normalisées et \textit{square-root} normalisées sont présentés à la première ligne. La seconde ligne contient les tracés de probabilités comparant les
...
\subsection{Méthodes de partitionnement de la distribution des Thêta-Bêta Ratios} \label{clustering}
...
Ce processus est répété jusqu'à ce que l'algorithme converge, menant à l'estimation de la \textit{maximum likelihood}. 

La même approche est suivie pour le \gls{bgmm} qui repose sur un algorithme d'inférence variationnelle (\textit{variational inference algorithm} en anglais), 
une variante de la technique d'\gls{em}, qui permet en plus une régularisation en intégrant des informations à partir de distributions d'\textit{a priori} \citep{Blei2006}.
Les \textit{a priori} utilisés ici sont les valeurs normalisées des \gls{tbr} des 74 sujets sains de la base de données \gls{cmi-hbn}, car on suppose que ces sujets
ont une activité cérébrale plutôt proche de celle des enfants \gls{tdah}.
...
Ce partitionnement hiérarchique agglomérant est représenté par un dendrogramme, dont un exemple est donné à la 
...
Ce seuil correspond à l'abscisse $x$ de l'intersection des deux Gaussiennes, c'est à dire qu'il faut trouver $x$ tel que $G_1(x) = G_2(x)$, avec $G_1$ et $G_2$ les deux Gaussiennes estimées. 
...
Le \gls{gmm} arrive à la même conclusion comme illustré à la Figure~\ref{Figure:tbr_gmm} : la distribution des \gls{srntbr} semble bimodale ($p$-value pour la comparaison
...
\begin{figure}[h!]
  \centering
	\includegraphics[width=1.0\linewidth]{figures/chapter-4/tbr-gmm} 
  \caption[Résultats des \gls{gmm} avec une à trois composantes.]{Tracés des distributions de \gls{srntbr} (\textit{rug plot}) avec en \textbf{A} 1 mode, en \textbf{B} 
	2 modes et en \textbf{C} 3 modes du \gls{gmm} superposés. $P$-value (1-2) correspond
	à la $p$-value du test de déviance comparant les \gls{gmm} à 1 mode et 2 modes ; $p$-value (2-3) correspond à la $p$-value du test de déviance comparant les \gls{gmm}
	à 2 modes et 3 modes. La significativité est symbolisée par ** et fixée à 1\%. Le seuil \gls{srntbr} est précisé pour le modèle à 2 composantes et représenté par un cercle bleu.}
  \label{Figure:tbr_gmm} 
\end{figure}
...
\begin{table}[h!]
  \centering
  \caption[Pourcentage de sujets considérés comme présentant un \glsfirst{tbr} élevé pour chaque seuil \gls{tbr} étudié.]{Pourcentage 
...
(12.5 - 25 Hz) isolées dans les zones frontales, temporales et centrales et aussi sur l'âge des sujets. Quatre groupes ont été identifiés : 
\begin{itemize}
...
plus adéquat, avec une \gls{acc} de 79.2\%. Toutefois, la courbe \gls{roc} étant calculée à partir du \gls{bgmm} à deux composantes, il était attendu que le
seuil obtenu avec ce partitionnement présente l'\gls{acc} la plus élevée. Par ailleurs, se baser sur l'\gls{acc} n'est réellement pertinent que lorsque le nombre 
d'observations dans les deux groupes estimés est le même, ce qui n'est pas le cas ici comme détaillé ensuite. Il serait alors préférable d'utiliser le \gls{tpr}.
...
de \gls{tbr} élevés comprenant seulement 5.79\% de la population. Ce faible pourcentage ainsi que le faible \gls{tpr} obtenu (18.5\%) laisse à penser
...
Au moment de choisir un seuil pour attribuer le protocole d'entrainement par \gls{nfb}, le seuil le plus précis n'est pas nécessairement le meilleur choix, surtout
dans notre cas où le nombre d'observations dans les groupes estimés est différent.
...
Au vu de ces résultats, nous recommandons l'utilisation d'un seuil \gls{tbr} de 4.1 pour attribuer
...
peuvent affecter les calculs de puissance et donc, par ricochet, les valeurs de \gls{tbr}. Ainsi, afin de qualifier l'impact de la correction des artefacts oculaires,
la distibution des valeurs de \gls{tbr} obtenues sans cette correction est comparée à celle obtenue sur les signaux corrigés. Les artefacts oculaires ont ici été corrigés 
à l'aide d'une méthode basée sur la \gls{bss} : l'\gls{ajdc} \citep{Barthelemy2017}. C'est une méthode
de correction non supervisée qui nécessite tout de même un enregistrement \gls{eegi} d'une minute contenant des artefacts oculaires. L'\gls{ajdc} est appliquée sur cet enregistrement 
et les caractéristiques des filtres et des sources obtenues sont par la suite utilisées lors de la correction des signaux \gls{eegi}.

Les distributions des valeurs de \gls{tbr} sans et avec 
la correction d'artefacts oculaires ont été tracées à la Figure~\ref{Figure:tbr_eye_artifact_correction} 
où aucune grande différence entre les deux distributions n'est observée. Par conséquent, la correction des artefacts 
...
avec l'âge des enfants, qu'ils souffrent du \gls{tdah} ou non \citep{Liechti2013, Snyder2015, Perone2018, Clarke2001sains}. De plus, le U-test de Mann-Whitney a été appliqué à nos données \gls{tbr} et a montré
...
entre 7 et 13 ans sont inclus, ce qui correspond à un total de 276 sujets \gls{tdah} et de 53 sujets sains. les résultats obtenus sur cette nouvelle population 
