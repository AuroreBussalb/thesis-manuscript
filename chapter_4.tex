...
grâce à des étapes de contrôle qui devaient validées pour pouvoir passer à l'enregistrement de l'\gls{eeg}.
...
variance (autrement dit l'électrode doit être déconnectée et n'enregistre rien) : dans les deux cas il est interpolé par \textit{spherical spline interpolation} \citep{Freeden1984}. 
...
les informations des \textit{a priori} sont intégrées. Ces \textit{a priori} sont les valeurs normalisées des \gls{tbr} des 74 sujets sains de la base de données \gls{cmi-hbn}.
...
à l'aide d'une méthode basée sur la \gls{bss} : l'\textit{approximate joint diagonalization of Fourier cospectra} (l'\gls{ajdc}) \citep{Barthelemy2017} qui est appliquée en temps 
réel. 
