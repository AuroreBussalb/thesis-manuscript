...
que ceux issus des bases de données du Child Mind Institute. Ainsi, une fois que chaque base a été décrite, les étapes
...
Le \gls{tbr} est un neuromarqueur couramment utilisé en \gls{nfb} appliqué aux enfants \gls{tdah} \citep{Arns2013}. Il est défini ici, puis les étapes pour l'extraire des
...
un chevauchement toutes les 0.125s) et la matrice de covariance de chaque segment est calculée pour un sous-ensemble de canaux. Ensuite, le \gls{rpf} rejette les 
...
(\textit{clustering} en anglais) 
sont mises en place : 
\begin{itemize}
\item une qui se base sur les distributions Gaussiennes (\gls{gmm} et \gls{bgmm}), 
...
La distribution des \gls{srntbr} est ainsi tracée et ajustée par un \gls{bgmm} \citep{Attias2000, Blei2006} puis par un \gls{gmm} pour comparaison. 
...
% trouver des refs
Les principaux facteurs de confusion ont été explorés, cependant il en existe d'autres qu'il n'est ici pas possible d'étudier faute de manque d'informations.
En effet, même si tous les enfants inclus dans cette étude sont diagnostiqués \gls{tdah}, ils ne présentent pas tous la même sévérité de symptômes.
