...
\begin{figure}[h!]
  ...
  \caption{Distribution des \gls{srntbr} répartis en \textbf{A)} : 2 \textit{clusters} (rouge et jaune) et \textbf{B)} : 3 \textit{clusters} 
	(vert, rouge et jaune). Le seuil séparant la distribution des \gls{srntbr} en deux groupes est représenté par la ligne verticale en pointilés rouges.}
...
où un coude est identifié à $y$ = 0.048, valeur choisie pour $\epsilon$.
...
Le groupe de \gls{srntbr} élevés (en jaune) est très dense : ces valeurs varient peu d'un sujet à l'autre.


\subsection{Seuils identifiés}
Chaque méthode permet de déterminer un seuil sur les \gls{srntbr} : pour obtenir le seuil sur les valeurs de \gls {tbr}, il suffit de calculer
le carré du seuil \gls{srntbr}. 

Le \gls{bgmm} à deux composantes, illustré à la Figure~\ref{Figure:tbr_bgmm}, est identifié comme ajustant le mieux la donnée : 
le seuil qui sépare le plus précisément les deux \textit{clusters} du \gls{bgmm} 
à deux composantes est de 1.925 pour les valeurs \gls{srntbr} ce qui correspond pour les valeurs \gls{tbr} à un seuil de 3.7 qui est très proche 
de celui obtenu avec le \gls{gmm} à deux modes (3.8). 

Le partitionnement en deux groupes avec la distance de Ward présenté à la Figure~\ref{Figure:tbr_ward_histograms} conduit à un seuil \gls{tbr} de 4.032.

En ce qui concerne le seuil \gls{tbr} identifié par l'algorithme \gls{dbscan}, il est de 6.426, une valeur bien plus élevée que pour les autres seuils
qui pourrait s'expliquer par le fait que 13 points ne sont attribués à aucun \textit{cluster}.

Le courbe \gls{roc} calculée sur le \gls{bgmm} à deux composantes ajustant les \gls{srntbr} est tracée à la Figure~\ref{Figure:tbr_roc} et présente une \gls{auc} de 0.872. 
Sur cette courbe sont ajoutés les seuils obtenus par les autres méthodes et l'\gls{acc} à laquelle ils conduisent. La valeur des seuils affichée correspond
au seuil sur les valeurs \gls{tbr}. 
...
79.2 \%. Deux seuils menant à des \gls{acc} bien plus faibles sont celui égal à 5 et celui trouvé par \gls{dbscan} avec respectivement une \gls{acc} de 71.4\%
et de 59.1\%. Toutefois, cette faible \gls{acc} est compensée par un plus faible taux de faux positifs.

La Table~\ref{Table:tbr_thresholds_number_of_subjects} résume le nombre de sujets attribués au groupe \gls{tbr} élevé en utilisant tous les seuils présentés
sur la courbe \gls{roc} à la Figure~\ref{Figure:tbr_roc}. La taille des groupes de \gls{tbr} élevés est assez stable, sauf pour le seuil identifié 
...
% à revoir
Trois méthodes de partitionnement différentes ont été utilisées afin de déterminer si un groupe d'enfants avec un \gls{tbr} existe et, auquel cas, quel
est le seuil à partir duquel le \gls{tbr} est considéré comme élevé. Cependant, cette analyse présente quelques limites qu'il est important de noter.
...
une \textit{log likelihood} un peu plus élevée (-204.790 contre -204.955), il n'y a pas de différence statistiquement significative entre ces dex modèles (p-value
...
la significativité de cette augmentation pour éviter l'\textit{overfitting}. Le modèle \gls{bgmm} à deux composantes décrit le mieux la distribution
...
des \gls{tbr} plus faibles. Lors de recherches précédentes sur l'identification de sous groupes chez les enfants \gls{tdah} basée sur l'\gls{eeg}, la méthode de
Ward a été utilisé \citep{Clarke2011} 
...
Par conséquent chacune de ces méthodes va dans le sens de l'existence d'un groupe de \gls{tbr} élevés, ce qui permet d'apporter de nouvelles preuves à
l'hétérogénéité du \gls{tdah} déjà évoquée dans la littérature \citep{Arns2008, Arns2012, Barry2003, Clarke2011, Liechti2013, Loo2013, Loo2018}.

Etant donné que le \gls{tdah} est un trouble hétérogène, tous les enfants \gls{tdah} ne vont pas nécessairement répondre au même traitement, ainsi personnaliser
le traitement pourrait être un moyen d'optimiser son efficacité pour chaque patient. L'existence de deux groupes distincts au sein de la 
population \gls{tdah} basé sur la valeur du \gls{tbr} étaye l'idée que le traitement par \gls{nfb} devrait être différent pour ces groupes. En effet, il semblerait
...
neuromarqueur, comme par exemple le \gls{smr}. Ce modèle à deux protocoles est une façon de personnaliser le traitement pour optimiser son efficacité. 
Cette personnalisation a été notamment suggérée par \citet{Kerson2013, Arns2012} et a été évaluée dans un récent essai clinique (NCT02778360, Mensia Technologies,
France, ClinicalTrials.gov, \citet{Bioulac2019}).

L'essai clinique de Mensia Technologies se base sur un seuil de \gls{tbr} = 4.5 pour assigner les sujets au groupe devant augmenter leur \gls{smr} ou au groupe 
devant diminuer leur \gls{tbr}. La courbe \gls{roc} représentée à la Figure~\ref{Figure:tbr_roc}, montre que choisir un seuil \gls{tbr} égal à 4.5 conduit
...
Le seuil calculé à partir du \gls{bgmm} à deux composantes comprend 36\% des sujets \gls{tdah}, formant le groupe des \gls{tbr} élevés. Ce pourcentage est
en accord avec les précédentes études qui concluent qu'entre 25 et 40\% des patients \gls{tdah} présentent un \gls{tbr} élevé \citep{Arns2012}. 
...
les protocoles d'entrainement du fait de son bon équilibre entre avoir un faible \gls{fpr} et une haute \gls{acc} et \gls{tpr}. 
